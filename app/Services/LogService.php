...
class LogService extends AppService
{
    ...
    public function getPagination(){
        // Get list
        $data = Log::where('account_id','=', $this->account_id)->where('direction','=', config('dashboard_constant.INBOUND'))->orderBy('log_time', 'DESC')->paginate(config('dashboard_constant.PAGINATION_LIMIT')); 
...
//->where('did','=', Auth::user()->cname)
     ...
     */
        ...
        ->select(DB::raw('COALESCE(SUM(num_contacts) - SUM(IF(num_sms_sent>num_contacts, num_contacts, num_sms_sent)),0) AS pending'))
        ->where('account_id', $accountId)
        ->where('start_time', ">", "2020-12-01 00:00:00")
        ->get();
    }
